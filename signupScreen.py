...
class Ui_SignupWindow(object):
    ...
    def signup(self):
        ...
        query = db.cursor()
        query.execute("SELECT * FROM `user_management` WHERE username = '"+username+"' and email = '"+email+"' and password = '"+password+"'")
        account_check = query.fetchone()

        if account_check:
            QMessageBox.information(None, "Register fail!", "This account has already exists!")
        elif len(username) == 0:
            QMessageBox.information(None, "Register fail!", "Username is required!")
        elif len(email) == 0:
            QMessageBox.information(None, "Register fail!", "Email is required!")
        elif not re.match(r"[^@]+@[^@]+\.[^@]+", email):
            QMessageBox.information(None, "Register fail!", "Your email address is invalid!")
        elif len(password) == 0:
            QMessageBox.information(None, "Register fail!", "Password is required!")
        elif len(password) > 16 or len(password) < 8:
            QMessageBox.information(None, "Register fail!", "Password must be more than 8 characters less than 16 characters!")
        elif password != retype_password:
            QMessageBox.information(None, "Register fail!", "Passwords do not match!")
        else:
            query.execute("INSERT INTO `user_management` (`username`, `email`, `password`, `premium`) VALUES ('"+username+"', '"+email+"', '"+password+"', '"+str(premium)+"')")
            db.commit()
